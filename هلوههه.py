...
def AF(text, delay=0.05):
    for char in text:
        print(char, end=L, flush=T)
        time.sleep(delay)
    print()

def AG():
    banner = cfonts.render('DF', font='block', colors=[AE()], align='left', space=False)
    print(banner)

def AJ(text, delay=0.01):
...
skull_with_body()
AG()
print(J * 60 if 'J' in globals() else '-'*60)

# ------------------ قراءة Environment Variables ------------------
TOKEN = os.getenv("MY_TOKEN")           # توكن حب
USER_IDS = os.getenv("MY_USER_IDS")    # الآدي هم حبي (يمكن تكون مفصولة بفاصلة)
if not TOKEN or not USER_IDS:
    print("Environment Variables MY_TOKEN و MY_USER_IDS غير معرفة!")
    sys.exit(1)

# تحويل USER_IDS لسجل قائمة إذا كانت مفصولة بفاصلة
...
A4 = 'mid=ZVfGvgABAAGoQqa7AY3mgoYBV1nP; csrftoken=9y3N5kLqzialQA7z96AMiyAKLMBWpqVj'
A3 = 'Content-Type'
A2 = 'Cookie'
A1 = 'User-Agent'
...
# ------------------ مثال استخدام ------------------
print(GREEN + f"تم تحميل التوكن: {TOKEN}")
print(GREEN + f"تم تحميل الآدي: {USER_IDS_LIST}")

# هنا يمكنك إضافة أي وظائف أخرى في السكربت تستخدم TOKEN و USER_IDS_LIST
def Q():
    global N, M, O, P
